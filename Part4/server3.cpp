...
void readSeqNumber(){
    ...
    ifstream ifs("server3_seq.txt"); // open the file 
    if (!ifs){
        cerr << "Can't open file: Server3_seq.txt";
        exit(0);
    }
...
void writeSeqNumber(){
    ...
    ofstream ofs;
    ofs.open("server3_seq.txt", ios::trunc); // open the file and overwrite its content
    if (!ofs){
        cerr << "Can't open file for writing: Server3_seq.txt";
        exit(0);
    }

...
#include <string>
    ...
    std::string deleteAccount(const std::string& id,const char tweetsFile [], const char usersFile [], const char friendFile [],const int port);
    
    // This method removes any line that start with the client ID in any file
    bool removeIDfromfile(const std::string& clientID, const char theFile[],const int port);
    ...
    bool removeIdFromYourFollowers(const std::string& clientID,const char friendsFile [],const int port);
